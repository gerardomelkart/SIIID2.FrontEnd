...
@Component({
  selector: 'app-federal-actualizacion',
  imports: [FormsModule],
  templateUrl: './federal-actualizacion.html',
...
export class FederalActualizacion implements OnInit {
  ...
  puedeConsultar = computed(() => this.anioCorte() !== '' && this.mesCorte() !== '');
  ...
  errorActualizacionPendiente = computed(() => {
    return (
      this.errores().find(
        (error) =>
          error.codigo === 'FEDERAL_ACTUALIZACION_PENDIENTE_EXISTENTE' ||
          error.codigo === 'FEDERAL_ACTUALIZACION_PENDIENTE_APROBACION',
      ) ?? null
    );
  });
  ...
  estadoActualizacionPendiente = computed(() => {
    ...
    return errorPendiente.codigo === 'FEDERAL_ACTUALIZACION_PENDIENTE_APROBACION'
  ...
  consultarPeriodo(): void {
    ...
    this.actualizacionService.consultarPeriodo(mes, anio).subscribe({
  ...
  async validarActualizacion(): Promise<void> {
  ...
  */
  ...
  private cargarPeriodosDisponibles(): void {
    ...
    this.actualizacionService.obtenerPeriodosDisponibles().subscribe({
      next: (periodos) => {
