...
} from '../../core/models/banci-usuarios.models';
...
@Component({
  selector: 'app-banci-usuarios',
  imports: [FormsModule],
  templateUrl: './banci-usuarios.html',
  styleUrl: './banci-usuarios.css',
})
export class BanciUsuarios implements OnInit {
  private readonly catalogosService = inject(CatalogosService);
  entidades = signal<EntidadFederativaCatalogoItem[]>([]);
  private readonly usuariosService = inject(BanciUsuariosService);
  ...
  ordenUsuarios = signal<EstadoOrden<CampoOrdenUsuariosBanci> | null>(null);

  usuarios = signal<BanciUsuarioDetalle[]>([]);
  formulario = signal<UsuarioBanciForm>(this.crearFormularioVacio());
  ...
  usuariosFiltrados = computed(() => {
    ...
    const filtrados = this.usuarios().filter((usuario) => {
      const pasaModulo = this.mostrarTodos() || usuario.habilitaBanci || (this.mostrarInactivos() && usuario.tieneBanci && !usuario.activo);
      const pasaBusqueda = !texto ||
        ...
        usuario.rol?.toLowerCase().includes(texto) || (usuario.entidadFederativa || 'Nacional').toLowerCase().includes(texto);
  ...
  formularioValido = computed(() => {
    ...
    if (!form.habilitaBanci && this.modoFormulario() === 'NUEVO') {
      return false;
    }
  ...
  ngOnInit(): void {
    this.cargarUsuarios();
    this.catalogosService.obtenerEntidadesFederativas().subscribe({ next: entidades => this.entidades.set(entidades.filter(e => e.idEntidadFederativa >= 1 && e.idEntidadFederativa <= 32)), error: () => mostrarError('No fue posible cargar las entidades', 'Intente recargar la página.') });
  }

  ordenarUsuariosPor(campo: CampoOrdenUsuariosBanci): void {
    this.ordenUsuarios.set(alternarOrden(this.ordenUsuarios(), campo));
    this.paginaUsuarios.set(1);
  }

  iconoOrdenUsuarios(campo: CampoOrdenUsuariosBanci): string {
  ...
  cargarUsuarios(): void {
    ...
    this.usuariosService.obtenerUsuarios(this.mostrarInactivos()).subscribe({
      ...
      error: (error) => {
        ...
        mostrarError(
          'No fue posible cargar usuarios Banci',
  ...
  async exportarUsuariosExcel(): Promise<void> {
    ...
    try {
      const filas = this.usuariosFiltrados().map((usuario) => ({
        ...
        Rol: usuario.rol,
        'Acceso Banci': usuario.habilitaBanci ? 'Sí' : 'No',
  ...
  abrirEditarUsuario(usuario: BanciUsuarioDetalle): void {
    if (!usuario.activo) {
      mostrarAdvertencia(
        'Usuario inactivo',
  ...
  cambiarEstado(usuario: BanciUsuarioDetalle): void {
  ...
  actualizarCampo<K extends keyof UsuarioBanciForm>(
    campo: K,
    valor: UsuarioBanciForm[K],
  ): void {
    ...
    if (campo === 'habilitaBanci' && valor === false) {
      this.formulario.update((actual) => ({
        ...actual,
        habilitaCarga: false,
  ...
  esUsuarioActual(usuario: BanciUsuarioDetalle): boolean {
  ...
  esUnicoSuperUsuarioActivo(usuario: BanciUsuarioDetalle): boolean {
    return (
      usuario.activo &&
      usuario.habilitaBanci &&
      usuario.rol === ROLES.SUPER_USUARIO &&
  ...
  puedeCambiarEstado(usuario: BanciUsuarioDetalle): boolean {
    if (usuario.rol === ROLES.SUPER_USUARIO) return false;
    if (!usuario.tieneBanci) return false;
  ...
  motivoBloqueoEstado(usuario: BanciUsuarioDetalle): string {
    if (usuario.rol === ROLES.SUPER_USUARIO) return 'Los superusuarios tienen acceso a todos los módulos activos';
    if (!usuario.tieneBanci) return 'Habilite el acceso Banci desde Editar usuario';
    ...
    if (this.esUnicoSuperUsuarioActivo(usuario)) {
      return 'No puedes desactivar el único superusuario Banci activo';
    }
  ...
  bloquearRolFormulario(): boolean {
    ...
    if (usuario.rol === ROLES.SUPER_USUARIO) return true;
  ...
  bloquearAccesoFormulario(): boolean {
    if (this.formulario().rol === ROLES.SUPER_USUARIO) return true;
  ...
  private obtenerValorOrdenUsuario(
    usuario: BanciUsuarioDetalle,
    campo: CampoOrdenUsuariosBanci,
  ): ValorOrden {
    return usuario[campo] ?? '';
  }

  private crearUsuario(form: UsuarioBanciForm): void {
    const request: CrearUsuarioBanciRequest = {
      ...
      rol: form.rol,
      idEntidadFederativa: form.idEntidadFederativa,
      habilitaBanci: form.habilitaBanci,
    ...
    this.usuariosService.crearUsuario(request).subscribe({
      next: (response) =>
        this.procesarGuardadoCorrecto(
          response,
          'Usuario creado correctamente en Banci.',
        ),
      error: (error) =>
        this.procesarErrorOperacion(
          error,
          'No fue posible crear el usuario Banci.',
        ),
    });
  }

  private editarUsuario(form: UsuarioBanciForm): void {
    if (!form.idUsuario) return;

    const request: EditarUsuarioBanciRequest = {
      usuario: form.usuario.trim(),
    ...
    this.usuariosService.editarUsuario(form.idUsuario, request).subscribe({
      ...
      error: (error) =>
        this.procesarErrorOperacion(
          error,
          'No fue posible actualizar el usuario Banci.',
        ),
    });
  }
  ...
  private async confirmarDesactivacion(
    ...
    const result = await confirmarAccion(
      'Desactivar usuario Banci',
      `Se desactivará a ${usuario.nombreCompleto} del módulo Banci. Sus accesos a otros módulos no serán modificados.`,
      'Desactivar',
    );
  ...
  private async confirmarReactivacion(
    ...
    const result = await confirmarAccion(
      'Reactivar usuario Banci',
      `Se reactivará a ${usuario.nombreCompleto} con acceso al módulo Banci. Los permisos de carga y actualización quedarán deshabilitados hasta que sean configurados.`,
      'Reactivar',
    );
  ...
  private procesarGuardadoCorrecto(
    response: BanciUsuarioOperacionResponse,
  ...
  private procesarErrorOperacion(
    ...
    const respuesta = (
      error as {
        error?: BanciUsuarioOperacionResponse;
      }
    )?.error;
  ...
  private mapearDetalleAFormulario(
    ...
    return {
      ...
      password: '',
      rol: usuario.rol ?? '',
      habilitaBanci: usuario.habilitaBanci,
  ...
  private crearFormularioVacio(): UsuarioBanciForm {
    return {
      idUsuario: null,
  ...
  private normalizarPermisosPorRol(rol: string): void {
    if (rol === ROLES.SUPER_USUARIO) { this.formulario.update(actual => ({ ...actual, idEntidadFederativa: null, habilitaBanci: true, habilitaCarga: true, habilitaModificacion: true })); return; }
