...
@Component({
  ...
  styleUrl: '../configuracion/configuracion.css',
})
export class SemanalConfiguracion implements OnInit {
  ...
  entidadesConfiguracion = computed<ConfiguracionEntidadSemanal[]>(() => {
    ...
    for (const usuario of this.usuarios()) {
      if (!usuario.activo || !usuario.habilitaSemanal || usuario.rol === ROLES.CONSULTA) continue;
    ...
    grupos.forEach((lista) => {
      ...
      const usuariosCarga = lista.filter((usuario) => usuario.habilitaCargaSemanal).length;
      const usuariosModificacion = lista.filter((usuario) => usuario.habilitaModificacionSemanal).length;
  ...
  entidadesFiltradas = computed(() => {
    ...
    const filtradas = !texto
      ...
      : this.entidadesConfiguracion().filter((entidad) => entidad.entidadFederativa.toLowerCase().includes(texto));

    return ordenarPorEstado(filtradas, this.ordenEntidades(), (entidad, campo) => entidad[campo] ?? '');
  });
  ...
  totalPaginasEntidades = computed(() => Math.max(1, Math.ceil(this.entidadesFiltradas().length / this.tamanioPaginaEntidades)));
  totalEntidades = computed(() => this.entidadesConfiguracion().length);
  totalEntidadesCargaActiva = computed(() => this.entidadesConfiguracion().filter((entidad) => entidad.estadoCarga === 'ACTIVO').length);
  totalEntidadesModificacionActiva = computed(() => this.entidadesConfiguracion().filter((entidad) => entidad.estadoModificacion === 'ACTIVO').length);
  ...
  cargarUsuarios(): void {
    ...
    this.usuariosService.obtenerUsuarios(true).subscribe({
      ...
      error: (error) => {
        this.cargando.set(false);
        mostrarError('No fue posible cargar configuración semanal', obtenerMensajeErrorHttp(error, 'Revise la conexión con la API.'));
      },
    });
  }
  ...
  async exportarConfiguracionExcel(): Promise<void> {
    ...
    try {
      ...
      const exportado = await exportarFilasExcel(filas, 'configuracion_semanal_por_entidad.xlsx', 'Configuracion');
  ...
  guardarConfiguracionGlobal(): void {
    const usuariosObjetivo = this.usuarios().filter((usuario) =>
      usuario.activo && usuario.habilitaSemanal && usuario.rol !== ROLES.CONSULTA,
    );

    if (usuariosObjetivo.length === 0) {
      mostrarInfo('Sin usuarios operativos', 'No existen usuarios activos con acceso al módulo preliminar.');
      return;
    }

    confirmarAccion(
      'Actualizar permisos semanales globales',
      `Se actualizarán los permisos semanales de ${usuariosObjetivo.length} usuario(s).`,
      'Sí, actualizar',
    ).then((result) => {
      ...
      const operaciones = usuariosObjetivo.map((usuario) =>
        this.usuariosService.actualizarPermisosSemanales(usuario.idUsuario, {
          ...
          catchError((error) => of({
            ...
            mensaje: obtenerMensajeErrorHttp(error, `No fue posible actualizar ${usuario.usuario}.`),
      ...
      forkJoin(operaciones).subscribe({
        next: (resultados) => {
          ...
          if (errores.length > 0) {
            mostrarAdvertenciaHtml('Algunos usuarios no se actualizaron', errores.map((error) => `• ${error.mensaje}`).join('<br>'));
            this.cargarUsuarios();
            return;
          }
        ...
        error: (error) => {
          this.guardandoGlobal.set(false);
          mostrarError('No fue posible actualizar configuración semanal', obtenerMensajeErrorHttp(error, 'Intente nuevamente.'));
        },
      });
    });
  }
  ...
  abrirPermisosEntidad(entidad: ConfiguracionEntidadSemanal): void {
    const usuariosEntidad = this.usuarios()
      .filter((usuario) => usuario.activo && usuario.habilitaSemanal)
  ...
  cambiarPermisoUsuarioEntidad(idUsuario: number, permiso: 'habilitaCarga' | 'habilitaModificacion', valor: boolean): void {
    this.usuariosEntidad.update((usuarios) =>
      usuarios.map((usuario) => usuario.idUsuario !== idUsuario || usuario.bloqueado ? usuario : { ...usuario, [permiso]: valor }),
    );
  }
  ...
  guardarPermisosEntidad(): void {
    ...
    confirmarAccion(
      'Guardar permisos semanales por entidad',
      ...
      const operaciones = usuariosModificados.map((usuario) =>
        this.usuariosService.actualizarPermisosSemanales(usuario.idUsuario, {
          habilitaSemanal: true,
          habilitaCargaSemanal: usuario.habilitaCarga,
          habilitaModificacionSemanal: usuario.habilitaModificacion,
          administraDelitosSemanal: usuario.administraDelitosSemanal,
        }).pipe(
          catchError((error) => of({
            esValido: false,
      ...
      forkJoin(operaciones).subscribe({
        next: (resultados) => {
          ...
          mostrarExitoInstitucional('Permisos semanales actualizados');
          this.cerrarPermisosEntidad();
          this.cargarUsuarios();
        },
        error: (error) => {
          this.guardandoEntidad.set(false);
          mostrarError('No fue posible actualizar permisos semanales', obtenerMensajeErrorHttp(error, 'Intente nuevamente.'));
        },
      });
    });
  }
  ...
  private sincronizarSwitchesGlobales(usuarios: UsuarioListadoItem[]): void {
    const usuariosOperativos = usuarios.filter((usuario) =>
      usuario.activo && usuario.habilitaSemanal && usuario.rol !== ROLES.CONSULTA,
    );
    ...
    this.habilitaCargaGlobal.set(usuariosOperativos.every((usuario) => usuario.habilitaCargaSemanal));
    this.habilitaModificacionGlobal.set(usuariosOperativos.every((usuario) => usuario.habilitaModificacionSemanal));
  }
}