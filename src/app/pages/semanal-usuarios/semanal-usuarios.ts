...
export class SemanalUsuarios implements OnInit {
  ...
  ordenUsuarios = signal<EstadoOrden<CampoOrdenUsuariosSemanales> | null>(null);
  ...
  formulario = signal<UsuarioSemanalForm>(this.crearFormularioVacio());
  ...
  formularioValido = computed(() => {
    ...
    if (!camposBaseValidos) return false;
    if (this.modoFormulario() === 'NUEVO' && form.password.trim() === '') return false;
    if (form.rol !== ROLES.SUPER_USUARIO && form.idEntidadFederativa === '') return false;
  ...
  abrirEditarUsuario(usuario: UsuarioListadoItem): void {
    if (!usuario.activo) {
      mostrarAdvertencia('Usuario inactivo', 'Reactive el usuario antes de editar sus datos.');
      return;
    }
  ...
  private crearUsuario(form: UsuarioSemanalForm): void {
    const request: CrearUsuarioSemanalRequest = {
    ...
    this.usuariosService.crearUsuarioSemanal(request).subscribe({
  ...
  private editarUsuario(form: UsuarioSemanalForm): void {
    if (!form.idUsuario) return;

    const request: EditarUsuarioSemanalRequest = {
      usuario: form.usuario.trim(),
  ...
  private confirmarDesactivacion(usuario: UsuarioListadoItem): void {
    confirmarAccion(
      'Desactivar usuario',
      `El usuario ${usuario.usuario} ya no podrá iniciar sesión ni operar los módulos del sistema.`,
      'Sí, desactivar',
    ).then((result) => {
  ...
  private confirmarReactivacion(usuario: UsuarioListadoItem): void {
    confirmarAccion(
      'Reactivar usuario',
      `El usuario ${usuario.usuario} volverá a estar activo con su configuración del módulo preliminar.`,
      'Sí, reactivar',
    ).then((result) => {
      if (!result.isConfirmed) return;
  ...
  private mapearDetalleAFormulario(usuario: UsuarioDetalle): UsuarioSemanalForm {
    return {
  ...
  private crearFormularioVacio(): UsuarioSemanalForm {
    return {
      idUsuario: null,
      nombre: '',
      primerApellido: '',
