    ...
    this.federalInformesService.obtenerPeriodosEnvios().subscribe({
      next: (periodos) => {
        ...
        const existe = periodos.some(
          (periodo) => `${periodo.anioCorte}-${periodo.mesCorte.toString().padStart(2, '0')}` === seleccionado,
        );

        if (!existe) {
          const periodo = periodos[0];
          this.periodoSeleccionado.set(`${periodo.anioCorte}-${periodo.mesCorte.toString().padStart(2, '0')}`);
        }
